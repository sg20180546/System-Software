#include "csapp.h"
#include <time.h>

#define MAX_CLIENT 100
#define STOCK_NUM 10
#define BUY_SELL_MAX 10

#define RAND 0x0
#define SHOW 0x1
#define MODIFY 0x2


void random_q();
void show_q();
void modify_q();

int order_per_client,clientfd,i;
char buf[MAXLINE],tmp[3];
rio_t rio;
int main(int argc, char **argv) 
{
	pid_t pids[MAX_CLIENT];
	int runprocess = 0, status;

	int num_client,total_order,query_type=RAND;
	char *host, *port;

	if (argc <5) {
		fprintf(stderr, "usage: %s <host> <port> <client#> <total_order> <query_type>\n", argv[0]);
		exit(0);
	}

	host = argv[1];
	port = argv[2];
	num_client = atoi(argv[3]);
	total_order=atoi(argv[4]);
	order_per_client=total_order/num_client;
	
	if(argv[5]){
		if(strcmp(argv[5],"show")){
			query_type=SHOW;
		}else if(strcmp(argv[5],"modify")){
			query_type=MODIFY;
		}
	}

/*	fork for each client process	*/
	while(runprocess < num_client){
		//wait(&state);
		pids[runprocess] = fork();

		if(pids[runprocess] < 0)
			return -1;
		/*	child process		*/
		else if(pids[runprocess] == 0){
			// printf("child %ld\n", (long)getpid());

			clientfd = Open_clientfd(host, port);
			Rio_readinitb(&rio, clientfd);
			srand((unsigned int) getpid());

			switch (query_type)
			{
			case SHOW:
				show_q();
				break;
			case MODIFY:
				modify_q();
				break;
			default:
				random_q();
				break;
			}


			Close(clientfd);
			exit(0);
		}
		/*	parten process		*/
		/*else{
			for(i=0;i<num_client;i++){
				waitpid(pids[i], &status, 0);
			}
		}*/
		runprocess++;
	}
	for(i=0;i<num_client;i++){
		waitpid(pids[i], &status, 0);
	}

	exit(0);
	/*clientfd = Open_clientfd(host, port);
	Rio_readinitb(&rio, clientfd);

	while (Fgets(buf, MAXLINE, stdin) != NULL) {
		Rio_writen(clientfd, buf, strlen(buf));
		Rio_readlineb(&rio, buf, MAXLINE);
		Fputs(buf, stdout);
	}

	Close(clientfd); //line:netp:echoclient:close
	exit(0);*/

	return 0;
}


void random_q(){
	for(i=0;i<order_per_client;i++){
		int option = rand() % 3;
				
		if(option == 0){//show
			strcpy(buf, "show\n");
		}
		else if(option == 1){//buy
			int list_num = rand() % STOCK_NUM + 1;
			int num_to_buy = rand() % BUY_SELL_MAX + 1;//1~10

			strcpy(buf, "buy ");
			sprintf(tmp, "%d", list_num);
			strcat(buf, tmp);
			strcat(buf, " ");
			sprintf(tmp, "%d", num_to_buy);
			strcat(buf, tmp);
			strcat(buf, "\n");
		}
		else if(option == 2){//sell
			int list_num = rand() % STOCK_NUM + 1; 
			int num_to_sell = rand() % BUY_SELL_MAX + 1;//1~10
					
			strcpy(buf, "sell ");
			sprintf(tmp, "%d", list_num);
			strcat(buf, tmp);
			strcat(buf, " ");
			sprintf(tmp, "%d", num_to_sell);
			strcat(buf, tmp);
			strcat(buf, "\n");
		}
			
		Rio_writen(clientfd, buf, strlen(buf));
		Rio_readnb(&rio, buf, MAXLINE);
	}
}

void modify_q(){
	for(i=0;i<order_per_client;i++){
		int option = rand() % 2;
		if(option == 0){//buy
			int list_num = rand() % STOCK_NUM + 1;
			int num_to_buy = rand() % BUY_SELL_MAX + 1;//1~10

			strcpy(buf, "buy ");
			sprintf(tmp, "%d", list_num);
			strcat(buf, tmp);
			strcat(buf, " ");
			sprintf(tmp, "%d", num_to_buy);
			strcat(buf, tmp);
			strcat(buf, "\n");
		}
		else if(option == 1){//sell
			int list_num = rand() % STOCK_NUM + 1; 
			int num_to_sell = rand() % BUY_SELL_MAX + 1;//1~10
					
			strcpy(buf, "sell ");
			sprintf(tmp, "%d", list_num);
			strcat(buf, tmp);
			strcat(buf, " ");
			sprintf(tmp, "%d", num_to_sell);
			strcat(buf, tmp);
			strcat(buf, "\n");
		}
			
		Rio_writen(clientfd, buf, strlen(buf));
		Rio_readnb(&rio, buf, MAXLINE);
	}
}

void show_q(){
	for(i=0;i<order_per_client;i++){
		strcpy(buf, "show\n");	
		Rio_writen(clientfd, buf, strlen(buf));
		Rio_readnb(&rio, buf, MAXLINE);
	}
}